...
fn test_bad_app_fails_rules() {
    ...
    assert!(has_errors, "Expected bad_app.ipa to trigger rule errors, but it passed cleanly.");
}
...
fn test_good_app_passes_rules() {
    ...
    assert!(!has_errors, "Expected good_app.ipa to pass all rules, but it triggered an error.");
}
